...
export async function fetchHealthStats(uid: string, days = 30): Promise<HealthStats> {
  ...
  // so ordering by date desc gives us the most recent records first.
  const q = query(
...
/**
 * Persist a minimal summary for the iOS widget via App Groups bridge
 * (StreakBridgeModule → UserDefaults) and AsyncStorage as fallback.
 */
